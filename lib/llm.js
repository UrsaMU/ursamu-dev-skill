...
export function validateBaseURL(raw) {
  ...
  // ::ffff:0:0/96 — IPv4-mapped IPv6 (e.g. ::ffff:7f00:1 → 127.0.0.1)
  // WHATWG normalises these to the ::ffff:hhhh:hhhh hex form.
  // Extract the embedded IPv4 address and re-run the private-range check.
  const v4mapped = ipv6raw.match(/^::ffff:([0-9a-f]{1,4}):([0-9a-f]{1,4})$/i);
  if (v4mapped) {
    ...
    if (isPrivateMapped) {
      throw new Error(
        `--base-url hostname "${ipv6raw}" is an IPv4-mapped IPv6 address that maps to ` +
        `${a}.${b}.${c}.${d}, a private or loopback range. ` +
        "SSRF to internal networks is not permitted."
      );
    }
  }
