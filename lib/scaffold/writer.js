...
import { assertNotOfficialName } from "../official-packages.js";
import {
  indexTemplate,
  commandsTemplate,
...
/**
 * Returns true if the current working directory looks like an UrsaMU project.
 * Checks for "ursamu" in package.json keywords/dependencies, or src/plugins/.
 */
export function isUrsamuProject(cwd = process.cwd()) {
  const pkgPath = join(cwd, "package.json");
  if (existsSync(pkgPath)) {
    try {
      const pkg = JSON.parse(readFileSync(pkgPath, "utf8"));
      const keywords = pkg.keywords ?? [];
      if (keywords.some(k => k.toLowerCase().includes("ursamu"))) return true;
      const deps = Object.keys({
        ...pkg.dependencies,
        ...pkg.devDependencies,
        ...pkg.peerDependencies,
      });
      if (deps.some(d => d.includes("ursamu"))) return true;
    } catch { /* ignore parse errors */ }
  }
  if (existsSync(join(cwd, "src", "plugins"))) return true;
  return false;
}

/**
 * Compute the relative path from the plugin output directory to deno.json at
 * the project root (process.cwd()). Returns null if deno.json doesn't exist.
 *
 * @param {string} absOutRoot - resolved absolute path of the plugin output dir
 * @returns {string|null}
 */
function resolveDenoJsonRelPath(absOutRoot, cwd = process.cwd()) {
  if (!existsSync(join(cwd, "deno.json"))) return null;
  return relative(absOutRoot, join(cwd, "deno.json")).replace(/\\/g, "/");
}
 ...
 * Refuses official @ursamu/* package slugs unless opts.force is true.
 *
 * @param {string} name
 * @param {{ force?: boolean }} [opts]
 * @throws {Error} if the name is invalid or reserved
 */
export function validateName(name, opts = {}) {
  ...
  assertNotOfficialName(name, { force: opts.force === true });
}
...
export function describeFiles(name, opts = {}) {
  const outRoot = opts.out ?? (isUrsamuProject() ? `./src/plugins/${name}` : `./${name}`);
  const files = [
    join(outRoot, "index.ts"),
    join(outRoot, "commands.ts"),
    join(outRoot, "README.md"),
    join(outRoot, "help", `${name}.md`),
  ];
 ...
 *
 * @param {string} name  - plugin name (re-validated here)
 * @param {object} opts
 ...
 * @param {boolean} [opts.force]       - allow official package names
 * @returns {string[]} - list of written absolute paths
 */
export function writeScaffold(name, opts = {}) {
  validateName(name, { force: opts.force === true });

  const outRoot = resolve(opts.out ?? (isUrsamuProject() ? `./src/plugins/${name}` : `./${name}`));
  ...
  const denoJsonRelPath = resolveDenoJsonRelPath(outRoot);
  write("index.ts",            indexTemplate(name, { ...opts, denoJsonRelPath }));
  write("commands.ts",         commandsTemplate(name));
  write("README.md",           readmeTemplate(name, opts));
  write(`help/${name}.md`,     helpFileTemplate(name));
