...
See also: check-03 catches a subset of known-bad operators ($push, $pull, etc.).
  ...
  "check-09": {
    ...
    level: "warn",
    text: `\
Import uses a bare "@ursamu/…" specifier without the "jsr:" prefix.

Why it matters:
  UrsaMU plugins run in Deno. Deno resolves bare npm-style specifiers only when
  deno.json (or package.json) explicitly maps them; without that mapping Deno
  throws "Module not found". This applies to every first-party package:
  @ursamu/mush, @ursamu/core, @ursamu/mail, @ursamu/bbs, @ursamu/combat,
  @ursamu/jobs, @ursamu/help, @ursamu/channels, and the rest of the catalog.
...
How to fix:
  ...
  // Good — jsr: scheme (self-contained)
  ...
  // Also good — map the bare name in deno.json imports
  // { "imports": { "@ursamu/mail": "jsr:@ursamu/mail@^2.4.0" } }

Auto-fixable: ursamu-audit --fix prefixes bare @ursamu/* imports with jsr:.
  Specifiers already listed in the nearest deno.json imports map are skipped.`,
  },
