 ...
 *
 * Fixable checks:
 *   check-09  from "@ursamu/<pkg>" → from "jsr:@ursamu/<pkg>"
 *             (any bare @ursamu/* specifier, including subpaths)
...
/**
 * Add jsr: scheme to bare @ursamu/* module specifiers on a single line.
 * Handles `from "…"`, `from '…'`, and side-effect `import "…"`.
 *
 * @param {string} line
 * @returns {string}
 */
export function prefixBareUrsamuImports(line) {
  return line.replace(
    /((?:from|import)\s+["'])(@ursamu\/[^"']+)(["'])/g,
    "$1jsr:$2$3"
  );
}
...
export function applyFixesToLines(lines, fixableViolations) {
  ...
  // ── check-09: add jsr: prefix to any bare @ursamu/* specifier ────────────
  ...
  for (const v of fixableViolations.filter(v2 => v2.check === "check-09")) {
    ...
    const fixed = prefixBareUrsamuImports(original);
    if (fixed !== original) {
      patched[idx] = fixed;
      applied++;
    }
