...
/**
 * Check if the nearest deno.json has an imports mapping for @ursamu/ursamu.
 * If it does, bare @ursamu/ursamu imports are valid.
 */
function hasImportMapForUrsamu(filePath) {
  ...
  try {
    ...
    while (dir && depth < 10) {
      ...
      if (existsSync(denoJsonPath)) {
        ...
        if (config && config.imports && config.imports["@ursamu/ursamu"]) {
          return true;
        }
      }
  ...
  try {
    const cwdDenoJson = join(process.cwd(), "deno.json");
    if (existsSync(cwdDenoJson)) {
      const config = JSON.parse(readFileSync(cwdDenoJson, "utf8"));
      if (config && config.imports && config.imports["@ursamu/ursamu"]) {
        return true;
      }
    }
...
export function checkImportPath(lines, filePath) {
  const violations = [];
  if (hasImportMapForUrsamu(filePath)) {
    return violations;
  }
  for (let i = 0; i < lines.length; i++) {
