...
/**
 * Walk upward from filePath (then cwd) and return the first deno.json `imports`
 * map found, or null.
 *
 * @param {string} [filePath]
 * @returns {Record<string, string>|null}
 */
export function findNearestDenoImports(filePath) {
  const tryRead = (denoJsonPath) => {
    try {
      if (!existsSync(denoJsonPath)) return null;
      const config = JSON.parse(readFileSync(denoJsonPath, "utf8"));
      if (config && config.imports && typeof config.imports === "object") {
        return /** @type {Record<string, string>} */ (config.imports);
      }
    } catch (_) {
      // ignore filesystem/JSON errors
    }
    return null;
  };
  ...
  if (filePath) {
    try {
      ...
      let depth = 0;
      while (dir && depth < 10) {
        const imports = tryRead(join(dir, "deno.json"));
  ...
  return tryRead(join(process.cwd(), "deno.json"));
}

/**
 * True when deno.json maps this bare @ursamu/* specifier (or a trailing-slash
 * prefix map), so the bare form is valid without a jsr: prefix.
 *
 * @param {string|null|undefined} filePath
 * @param {string} specifier  e.g. "@ursamu/mail" or "@ursamu/mush/app"
 * @returns {boolean}
 */
export function hasImportMapForUrsamuSpecifier(filePath, specifier) {
  if (!specifier || !specifier.startsWith("@ursamu/")) return false;
  const imports = findNearestDenoImports(filePath ?? undefined);
  if (!imports) return false;

  if (imports[specifier]) return true;
  if (imports[specifier + "/"]) return true;

  // Prefix map: "@ursamu/mail/" covers "@ursamu/mail/foo"
  for (const key of Object.keys(imports)) {
    if (key.endsWith("/") && specifier.startsWith(key)) return true;
  }

  // Bare package root map covers subpaths: "@ursamu/mush" → "@ursamu/mush/app"
  const slash = specifier.indexOf("/", "@ursamu/".length);
  if (slash !== -1) {
    const root = specifier.slice(0, slash);
    if (imports[root] || imports[root + "/"]) return true;
  }
...
// Flags bare `@ursamu/*` imports missing the `jsr:` prefix in Deno context.
// Covers engine (@ursamu/mush, @ursamu/ursamu, @ursamu/core) and feature
// packages (@ursamu/mail, @ursamu/bbs, @ursamu/combat, …).
// Skipped per-specifier when deno.json imports maps that bare name.
// Confidence: MEDIUM — import maps make bare forms valid.

/**
 * Match bare @ursamu/* string literals used as module specifiers.
 * Captures the specifier including optional subpath (e.g. @ursamu/mush/app).
 * Does not match jsr:@ursamu/… (those already have a scheme prefix).
 */
export const BARE_URSAMU_SPECIFIER_RE =
  /(?:from\s+|import\s+)["'](@ursamu\/[^"']+)["']/g;

/**
 * @param {string} line
 * @returns {string[]} bare @ursamu/* specifiers found on the line
 */
export function findBareUrsamuSpecifiers(line) {
  const found = [];
  BARE_URSAMU_SPECIFIER_RE.lastIndex = 0;
  let m;
  while ((m = BARE_URSAMU_SPECIFIER_RE.exec(line)) !== null) {
    found.push(m[1]);
  }
  return found;
}
...
export function checkImportPath(lines, filePath) {
  const violations = [];
  for (let i = 0; i < lines.length; i++) {
    const specs = findBareUrsamuSpecifiers(lines[i]);
    for (const spec of specs) {
      if (hasImportMapForUrsamuSpecifier(filePath, spec)) continue;
      violations.push({
        file: filePath, line: i + 1, check: "check-09", level: "warn",
        message:
          `Import uses "${spec}" without the "jsr:" prefix. ` +
          `Deno requires: import ... from "jsr:${spec}" ` +
          `(or map "${spec}" in deno.json imports).`,
      });
    }
  }
  return violations;
...
// Detects exec() blocks that call util.target() and write to the DB but have
// no canEdit() guard. The SDK does NOT enforce write permission automatically.
// Confidence: MEDIUM — canEdit may live in a shared helper function.

const CANEDIT_RE = /\bcanEdit\s*\(/;
 ...
 * @returns {Violation[]}
 */
export function checkCanEditGuard(lines, filePath) {
  const blocks = extractAllBlocks(lines, EXEC_BLOCK_RE);
  const violations = [];

  for (const block of blocks) {
    const combined = block.bodyLines.map(l => l.text).join("\n");
    if (TARGET_CALL_RE.test(combined) && DB_WRITE_RE.test(combined) && !CANEDIT_RE.test(combined)) {
      violations.push({
        file: filePath, line: block.headerLine, check: "check-02", level: "warn",
        message:
          "exec() fetches a target and writes to the DB but no canEdit() guard found. " +
          "Call await u.canEdit(u.me, target) before modifying another object.",
      });
    }
  }
  return violations;
...
// Detects addCmd blocks with admin/wizard lock but no explicit flags.has() check.
// Confidence: LOW — some admin commands intentionally rely on lock: alone.

const ADMIN_LOCK_RE  = /lock\s*:\s*["'][^"']*\b(admin|wizard)\b[^"']*["']/;
const FLAGS_CHECK_RE = /\.flags\s*\.\s*(has|get)\s*\(/;
 ...
 * @returns {Violation[]}
 */
export function checkAdminFlagsGuard(lines, filePath) {
  const blocks = extractAllBlocks(lines, ADDCMD_RE);
  const violations = [];

  for (const block of blocks) {
    const combined = block.bodyLines.map(l => l.text).join("\n");
    if (!ADMIN_LOCK_RE.test(combined)) continue;
    if (!FLAGS_CHECK_RE.test(combined)) {
      violations.push({
        file: filePath, line: block.headerLine, check: "check-05", level: "hint",
        message:
          "addCmd() uses an admin/wizard lock but exec() does not call .flags.has(). " +
          "The lock: field guides routing but runtime state may differ — validate flags explicitly in exec().",
      });
    }
  }
  return violations;
...
// Detects lines with MUSH color codes that do not include a %cn reset.
// Confidence: MEDIUM — false positives on multi-line strings are possible.

const COLOR_START_RE = /%c[hrgbywcmu]/i;
const COLOR_RESET_RE = /%cn/i;
 ...
 * @returns {Violation[]}
 */
export function checkColorReset(lines, filePath) {
  const violations = [];
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (/^\s*(\/\/|\/\*|\*)/.test(line)) continue;
    if (COLOR_START_RE.test(line) && !COLOR_RESET_RE.test(line)) {
      violations.push({
        file: filePath, line: i + 1, check: "check-07", level: "warn",
        message:
          "Line contains a MUSH color code without a %cn reset. " +
          "Always end colored strings with %cn to reset terminal attributes.",
      });
    }
  }
  return violations;
...
// Complements check-03: flags any db.modify op not in {$set, $inc, $unset}.
// Confidence: MEDIUM — op string may appear on a subsequent line.

const APPROVED_OPS = new Set(["$set", "$inc", "$unset"]);
const OP_STRING_RE  = /,\s*["'](\$[a-zA-Z]+)["']/;
 ...
 * @returns {Violation[]}
 */
export function checkDbModifyOp(lines, filePath) {
  const violations = [];
  for (let i = 0; i < lines.length; i++) {
    if (!DB_MODIFY_RE.test(lines[i])) continue;
    const window = lines.slice(i, Math.min(i + 5, lines.length)).join(" ");
    const m = OP_STRING_RE.exec(window);
    if (m && !APPROVED_OPS.has(m[1])) {
      violations.push({
        file: filePath, line: i + 1, check: "check-08", level: "error",
        message:
          `db.modify() called with unapproved op "${m[1]}". ` +
          'Only "$set", "$inc", and "$unset" are permitted.',
      });
    }
  }
  return violations;
...
export function runAllChecks(lines, filePath) {
  return [
    ...checkInputSanitization(lines, filePath),    // check-01
    ...checkCanEditGuard(lines, filePath),          // check-02
    ...checkAtomicDbWrites(lines, filePath),        // check-03
    ...checkTargetNullGuard(lines, filePath),       // check-04
    ...checkAdminFlagsGuard(lines, filePath),       // check-05
    ...checkSandboxSafety(lines, filePath),         // check-06
    ...checkColorReset(lines, filePath),            // check-07
    ...checkDbModifyOp(lines, filePath),            // check-08
    ...checkImportPath(lines, filePath),            // check-09
    ...checkHelpText(lines, filePath),              // check-10
    ...checkPluginPhaseDiscipline(lines, filePath), // check-11
    ...checkGameHooksPairing(lines, filePath),      // check-12
    ...checkDboNamespace(lines, filePath),          // check-13
    ...checkRestAuthGuard(lines, filePath),         // check-14
    ...checkInitReturnsTrue(lines, filePath),       // check-15
  ];
}
